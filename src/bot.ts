...
bot.start(async (ctx: ITextMessageContext & { startPayload: string }) => {
  ...
  await ctx.replyWithHTML(
    [
      `Привет! 👋`,
      `Бот позволяет подписаться на уведомления об изменениях в <a href="${xEnv.YSTUTY_PRKOM_URL}/">списках <s>поступающих</s> к зачислению ЯГТУ</a>.`,
...
const onHelp = async (ctx: ITextMessageContext) => {
  ctx.replyWithHTML(
    [
      ...
      `  • 🔴 — Недостаточно баллов (баллы еще не были проставлены или не были указаны при подаче заявления) / Не проходит в списках <s>для поступления</s> к зачислению`,
      `  • 🟡 — Есть шанс (баллов для зачисления достаточно)`,
      `  • 🟢 — Абитуриент точно проходит на специальность / Уже зачислен`,
      ``,
      `• <b>Общая Позиция</b> — обычная позиция в списке, рассчитанная по баллам.`,
  ...
  ctx.replyWithHTML(
    [
      `<b>ℹ️ Информация:</b> <i>позиция в списке <s>поступающих</s> к зачислению не всегда определяет возможность зачисления на выбранную специальность.</i>`,
      ``,
...
bot.help(onHelp);
bot.hears(
  new RegExp(`^${keyboardFactory.KeyboardKeys.main.help}`, 'i'),
  onHelp as any,
);
